...
// standard employee engagement / comms-audit questions. All figures, and the
// people themselves, are illustrative (PoC only), not real survey output or
// real employees.
...
export const PERSONAS = [
  {
    ...
    audit: {
      ...
      topFrustration: "Too many messages arriving through channels she doesn't check regularly.",
    },
  },
  {
    ...
    audit: {
      ...
      whatWorksWell: "Short team briefings where he can ask questions on the spot.",
      topFrustration: "Finding out about changes from a colleague before his manager tells him.",
    },
  },
  {
    id: "social-connector",
    ...
    audit: {
      ...
      whatWorksWell: "Informal updates she can react to, comment on, or ask follow-up questions about.",
  ...
  },
  {
    ...
    audit: {
      ...
      whatWorksWell: "Concise updates that respect his time and aren't repeated verbatim everywhere.",
  ...
  },
  {
    ...
    audit: {
      ...
      whatWorksWell: "Nothing scored highly — the lowest-friction option is usually whatever her manager repeats most.",
      topFrustration: "Comms that feel like they're written for head office, not for her role.",
    },
  },
  {
    ...
    audit: {
      ...
      whatWorksWell: "Short videos and posters placed where he already gathers (canteen, shift changeover).",
      topFrustration: "Being sent long text documents with no easy way to read them on shift.",
    },
  },
];
...
export function buildPersonaSystemPrompt(persona) {
  ...
  return [
    `You are roleplaying as ${persona.name}, an employee persona representing ${persona.size}% of the workforce in an internal communications & employee engagement segmentation study. Internally this segment is nicknamed "${persona.archetype}", but that's a research label, not something ${persona.name} would call herself/himself — stay in character as ONE typical employee from this segment, speaking in first person, informally, as a real employee would in a quick chat, not as a corporate summary.`,
    ``,
    `Your profile:`,
    `- Segment nickname (research label only): "${persona.archetype}"`,
