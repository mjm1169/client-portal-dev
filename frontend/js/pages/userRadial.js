...
export function mountUserRadial(container, user) {
...
function showError(el, html) {
  el.innerHTML = `<span class="upload-error-icon" aria-hidden="true">⚠</span> ${html}`;
...
function drawChart(data, scores) {
  ...
  const firstScore = scores[0]?.id || 'Score1';
  ...
  const path = pathGroup
    ...
    .attr("fill", d => {
      const v = d.data.scores?.[firstScore];
      return (v == null || isNaN(v)) ? "#ddd" : colorScale(v);
    })
    .attr("stroke", "#fff")
  ...
  const centerText = svg
    ...
    .text(`${root.data.name}: ${root.data.scores[firstScore] || 0}%`);
  ...
  const label = labelGroup
    ...
    .text(d =>
      (d.data.scores[firstScore] || 0) > threshVal
        ? `${d.data.name}: ${d.data.scores[firstScore] || 0}%`
